...
const fetchRegisterBigshareonline = async () => {
    try {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (company_id && company_id !== "--Select Company--") {
                await IpoModel.create({ company_id, company_name, registrar_name: "bigshareonline" });
            }
        }
        return;
    } catch (error) {
...
export default fetchRegisterBigshareonline;