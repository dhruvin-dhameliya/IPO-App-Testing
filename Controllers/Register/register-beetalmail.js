...
const fetchRegisterBeetalmail = async () => {
    try {
        ...
        const options = $('#ctl00_CPHDetails_DropDownList1 option');
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (isExist) break;
            if (company_id) {
                await IpoModel.create({ company_id, company_name, registrar_name: "beetalmail" });
            }
        }
        return;
    } catch (error) {
...
export default fetchRegisterBeetalmail;